...
const Auth = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await axios.post(`${URL}/${isSignup ? 'signup' : 'login'}`, {
      fullName,
      username,
      password,
      phoneNumber,
    ...
    if (result.status === 200 && !!result.data.data) {
      const { token, username, fullName, userId } = result.data.data;
      ...
      if (isSignup) {
        cookies.set("hashedPassword", result.data.data.hashedPassword);
